...
import fr.hb.ewan.plages.dao.ReservationDao;
import fr.hb.ewan.plages.dao.StatutDao;
...
public class AjoutDonneesInitiales implements CommandLineRunner {
	...
	// La passwordEncoder a été instantié par une méthode annotée @Bean, dans PlageApplication
	// C'est la méthode encoder
	...
	public void ajouterClientsSansCondition(int nbClientsAAjouter) {
		...
		while (map.size() != nbClientsAAjouter) {
			...
			// client.setMotDePasse(passwordEncoder.encode(String.valueOf(random.nextInt(99999999) + 10000000)));
	...
	private void ajouterClients(int nbClientsAAjouter) {
		if (clientDao.count() == 0) {
			ajouterClientFixeSansCondition();
	...
	private void ajouterClientFixeSansCondition() {
			Client client = new Client();
			client.setNom("PIVOT");
			client.setPrenom("Marcel");
			client.setEmail("marcel.pivot@humanbooster.fr");
			client.setMotDePasse(passwordEncoder.encode("87654321"));
			client.setDateHeureInscription(LocalDateTime.of(2023, 8, 10, 12, 0));
			client.setPays(paysDao.findByCode("FR"));
	        client.setLienDeParente(lienDeParenteDao.findByNom("Cousin/Cousine"));
	...
	private void ajouterReservations(int nbReservationsAAjouter) {
		...
		for (int i = 0; i < nbReservationsAAjouter; i++) {
			LocalDate dateDebut = LocalDate.of(2023, random.nextInt(3) + 6, random.nextInt(30) + 1);
